...
def actionManagerKey(currentMap, playerPosX, playerPosY, inventory, skills, screen, activeQuests, modulePositions, textToDisplay, prayerPoints, key):
    textToDisplay = []
...
def keyboardManager(screen):
    wait = True
    while True:
        key = screen.getch()
        if not key == -1 and not wait:
            return key
        else:
            wait = False